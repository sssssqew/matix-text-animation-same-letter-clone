...
function displayLetter(){ // 랜덤글자를 생성하고 화면에 보여주기 위한 타이머 설정
  ...
  const letter = createLetter(randomLetter, pickRandomNumber(widthOfBrowser)) // 랜덤한 위치에 랜덤한 글자 생성 (브라우저 너비를 벗어나지 않는 범위에서)
  ...
  setInterval(function(){ // 생성한 랜덤글자를 Y축으로 이동시키기 위한 타이머 설정 
    ...
    const cloneLetter = createLetter(randomLetter, letter.offsetLeft, letter.offsetTop) // 처음에 생성한 랜덤글자와 동일한 글자를 현재 Y측 위치에 복제
